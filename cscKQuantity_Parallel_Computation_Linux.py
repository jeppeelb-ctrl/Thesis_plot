...
def generate_pic_gen_lists(picard_group, fan, coho):
    ...
    This function creates the generator list of the Picard group, which is the divisor classes [1,0,...]
        ...
        representation of the torus invariant divisors corresponding to the rays of the fan.

    picard_group: is the Picard group computed by Sagemath
...
def first_factor(alpha, beta, intersection_matrix, fan, gen_list_count, gen_list):
    '''
    ...
    numerator = intersection_calculator(tuple(alpha), tuple(beta), intersection_matrix, fan, gen_list_count, gen_list)
    denominator = intersection_calculator(tuple(alpha), tuple(alpha), intersection_matrix, fan, gen_list_count, gen_list)
    if denominator == 0:
        return 0
    else:
...
def second_factor(alpha, beta, C, intersection_matrix, fan, gen_list_count, gen_list):
    '''
    ...
    numerator = intersection_calculator(tuple(beta), tuple(C), intersection_matrix, fan, gen_list_count, gen_list)
    denominator = intersection_calculator(tuple(alpha), tuple(C), intersection_matrix, fan, gen_list_count, gen_list)
    if denominator == 0:
        return None
    else:
...
def convert_to_coho(coefficients, gen_list_count, fan):
    ...
    Returns: the representation of a divisor using the torus invariant divisors
    '''
...
def convert_C_to_ray_divisor(C):
    ...
    Returns: a list of the form [0,...,1,...,0] representing the torus invariant divisor
    '''
...
def optim_heat_map_CSCK(alpha, beta, intersection_matrix, C_div_list, fan, gen_list_count, gen_list):
    '''
    This function computes the maximum value of the cscK quantity I(α) checking for
        each torus invariant divisor

    alpha: is the divisor corresponding to the alpha part of the quantity I(α)

    beta: is the divisor corresponding to the beta part of the quantity I(α)

    Returns: a list with the maximum value at index 0,
             and the torus invariant divisor that realised the maximum value at index 1
    '''
    div_by_zero = []
    value = -float(50000)
    C_div = []
...
def compute_intersection_matrix(number_of_rays, X, coho):
    ...
    This function computes the intersection matrix of the torus invariant divisors
        corresponding to the rays of the fan, using only Sagemath
...
def process_alpha(alpha_indices, beta, intersection_matrix, Kc, C_div_list, fan, gen_list_count, gen_list, size_of_matrix):
    ...
    if Kc.contains(tuple(alpha)):
        result = optim_heat_map_CSCK(alpha, beta, intersection_matrix, C_div_list, fan, gen_list_count, gen_list)
        value = result[0]
